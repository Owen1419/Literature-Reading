...
$$
B_{j,0}(x)=1\quad\mathrm{for}\quad \tau_j\le x<\tau_{j+1};
\qquad B_{j,0}(x)=0\quad\mathrm{otherwise}.
$$
...
$$
\mathbf y=\mathbf B\mathbf c+\varepsilon,
...
$$
w_{im}^{(\lambda)}=\frac{1}{2}[1+\cos(\pi\frac{|\lambda_i-\lambda_m|}{h_\lambda})]
\quad\mathrm{for}\quad |\lambda_i-\lambda_m|\le h_\lambda;
\qquad w_{im}^{(\lambda)}=0\quad\mathrm{otherwise}.
$$
...
w_{im}^{(\lambda)}w_i^{(q)}
[V_i-\sum_jc_{mj}B_{j,3}(x_i)]^2
+\alpha\int_0^1[\widehat V_m''(x)]^2dx
\rbrace.
$$
...
$$
\beta\sum_m\sum_r
[\widehat V_m(\phi_r)-\widehat V_{m+1}(\phi_r)]^2,
$$
...
$$
W_\lambda\in\lbrace 0.50^\circ,0.75^\circ,1.00^\circ\rbrace.
$$
...
$$
\mathcal K=\lbrace 0,1,2,3,4,5,6,7\rbrace.
$$
...
\sqrt{\frac{1}{N_{test}}
\sum_{i\in test}
[V_i-\widehat V_{-g(i),K}(\lambda_i,\phi_i)]^2},
$$
...
$$
\mathcal K_{good}=
\lbrace K\in\mathcal K:\; E_{CV}(K)\le E_{min}+SE(E_{min})\rbrace.
$$
...
[\frac{d^2\widehat V_{m,K}(\phi)}{d\phi^2}]^2d\phi.
$$
...
$$
\mathrm{CV\ prediction\ accuracy}
\;\to\;
\mathrm{avoid\ unnecessary\ complexity}
\;\to\;
\mathrm{minimize\ nonphysical\ gradient\ oscillation}.
$$
